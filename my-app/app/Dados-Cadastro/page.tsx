...
interface TipoCadastro {
    idCliente: number; // Alterado para idCliente
    usuario: string;
    nome: string;
    email: string;
    senha: string;
...
export default function Cadastro() {
    ...
    const [cadastro, setCadastro] = useState<TipoCadastro>({
        idCliente: 0, // Inicializando com 0, que deve ser um valor válido para o ID
        usuario: "",
        nome: "",
        email: "",
        senha: "",
    ...
    const [modoEdicao, setModoEdicao] = useState<number | null>(null); // Usando número para o ID
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setCadastro({ idCliente: 0, usuario: "", nome: "", email: "", senha: "", placa: "" }); // Resetando cadastro
                setModoEdicao(null);
    ...
    const handleDelete = async (idCliente: number) => { // Recebendo idCliente
        try {
            const response = await fetch(`http://localhost:8080/usuario/${idCliente}`, {
                method: 'DELETE',
            });

            if (response.ok) {
                setCadastros(cadastros.filter((c) => c.idCliente !== idCliente));
    ...
    const iniciarEdicao = (cadastro: TipoCadastro) => {
        ...
        setModoEdicao(cadastro.idCliente); // Definindo o ID correto
        window.scrollTo(0, 0);
    };
        ...
        <div className='bg-black'>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col w-full space-y-3">
                        ...
                        {modoEdicao && (
                            <button
                                ...
                                onClick={() => {
                                    setModoEdicao(null);
                                    setCadastro({ idCliente: 0, usuario: "", nome: "", email: "", senha: "", placa: "" });
                                }}
            ...
            <div className="mt-10 bg-black max-w-6xl mx-auto py-10 px-5">
                ...
                <div className="flex flex-wrap justify-center gap-6">
                    {cadastros.map((dev) => (
                        <div key={dev.idCliente} className="bg-white p-5 rounded-lg shadow-lg text-black">
                            ...
                            <div className="flex justify-between mt-5">
                                ...
                                <button
                                    onClick={() => handleDelete(dev.idCliente)}
